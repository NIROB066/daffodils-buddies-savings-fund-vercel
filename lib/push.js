...
/** Construct a recipient-personalized push payload (title, mention state, tag). */
function personalizePayload(target, payload) {
  if (!payload || typeof payload !== 'object') return payload;
  const recipientName = target.name || '';
  const who = payload.member || 'Buddies';
  const isAll = !!payload.isAll;
  const isMeMentioned = isAll || (Array.isArray(payload.mentions) && payload.mentions.includes(recipientName));

  let title = payload.title;
  if (!title) {
    if (payload.type === 'post') {
      if (isAll) title = `${who} mentioned @all in a post 📝`;
      else if (isMeMentioned) title = `${who} mentioned you in a post 📝`;
      else title = `${who} shared a post 📝`;
    } else if (payload.type === 'photo') {
      if (isAll) title = `${who} mentioned @all in a memory 📸`;
      else if (isMeMentioned) title = `${who} mentioned you in a memory 📸`;
      else title = `${who} shared a new memory 📸`;
    } else {
      // chat message
      if (isAll) title = `${who} mentioned @all 📢`;
      else if (isMeMentioned) title = `${who} mentioned you 💬`;
      else title = `${who} · Daffodils Buddies`;
    }
  }
  ...
  return {
    ...payload,
    title,
    mentioned: isMeMentioned,
    tag: payload.tag || (payload.type === 'post' ? 'daf-post' : payload.type === 'photo' ? 'daf-photo' : 'daf-chat'),
  };
}
...
function sendTo(targets, payload) {
  ...
  return Promise.all(targets.map((r) => {
    const personalized = personalizePayload(r, payload);
    const body = JSON.stringify(personalized);
    return webpush.sendNotification(toSubscription(r), body)
