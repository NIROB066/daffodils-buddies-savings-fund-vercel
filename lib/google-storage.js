...
/** Raw cell arrays for every tab, allowing callers to inspect or repair headers. */
async function readTabsRaw(tabs) {
  const out = new Map();
  if (!tabs.length) return out;
  const { sheets } = getClients();
  const result = await withRetry(() => sheets.spreadsheets.values.batchGet({
  ...
  const ranges = result.data.valueRanges || [];
  tabs.forEach((tab, index) => out.set(tab, ranges[index]?.values || []));
  return out;
}

/** Creates any missing tabs, writes their header rows immediately, and reports populated tabs. */
async function ensureTabs(tabs, columnsByTab = {}) {
  const { sheets } = getClients();
  ...
  if (missing.length) {
    ...
    // Immediately write header row to newly created sheets so they are never header-less
    for (const tab of missing) {
      const cols = columnsByTab[tab];
      if (cols && cols.length) {
        await writeTab(tab, [], cols);
      }
    }
  }
...
module.exports = { configured, ensureTabs, readTab, readTabs, readTabsRaw, writeTab, appendTab };
