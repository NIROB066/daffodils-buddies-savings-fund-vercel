...
async function initGoogleStorage(columns) {
  if (!usingGoogle() || googleRows.size) return;
  const tabs = Object.keys(columns).map((name) => googleTab(`${name}.csv`));
  await googleStorage.ensureTabs([...new Set(tabs)]);
  await Promise.all(Object.keys(columns).map(async (name) => {
    googleRows.set(name, await googleStorage.readTab(googleTab(`${name}.csv`)));
  }));
}
